  ...
  async fetch(request:Request,env:V3Env):Promise<Response>{
    ...
    try{
      if(url.pathname==="/health"&&request.method==="GET"){
        const started=performance.now();await ensureBootstrap(env);
        const [students,progress,targets,homework,generatedHomework,dummy]=await env.DB.batch([env.DB.prepare("SELECT COUNT(*) AS count FROM students"),env.DB.prepare("SELECT COUNT(*) AS count FROM v3_progress_records"),env.DB.prepare("SELECT COUNT(*) AS count FROM v3_target_snapshot"),env.DB.prepare("SELECT COUNT(*) AS count FROM v3_homework_snapshot"),env.DB.prepare("SELECT COUNT(*) AS count FROM v3_homework_items"),env.DB.prepare("SELECT COUNT(*) AS count FROM students WHERE student_id='1320'")]);
        const count=(r:D1Result<unknown>)=>Number((r.results[0] as Row|undefined)?.count||0);
        return json({ok:true,service:"step-progress-v3-staging",mode:"d1-isolated-autosave",studentCount:count(students),progressCount:count(progress),targetCount:count(targets),homeworkCount:count(homework),generatedHomeworkCount:count(generatedHomework),dummy1320:count(dummy),bootstrapMs:elapsed(started)});
      }
