...
// Keep the login pane hidden while a valid remembered session is being resumed,
// but never hide it after an explicit manual logout.
...
// Paint the progress hero immediately from either cached or freshly returned
// dashboard data. A bad old cache is discarded and re-fetched instead of
// leaving the graph blank indefinitely.
...
const earlyPatch = "if(eagerDashboardPromise)eagerDashboardPromise.then(()=>renderStudent()).catch(error=>console.error('[dashboard-load-failed] '+String(error?.message||error)));";
...
if (!html.includes(earlyPatch)) throw new Error('dashboard completion render patch missing');

// On ordinary refresh, a still-valid app session wins over a stale shared
// session. This prevents Ctrl+R/F5 from sending the user back to login.
...
// Homework rendering replaces the main shell. Put the progress hero back from
// cache immediately after either homework renderer completes.
...
if (!html.includes('renderProgressHeroFast_(out.data)')) throw new Error('fresh dashboard does not paint immediately');
if (!html.includes('state.dashboardCache&&!renderProgressHeroFast_(state.dashboardCache)')) throw new Error('cached dashboard does not paint immediately');
...
console.log('Applied V3 graph/session hotfix; cached/fresh graph paints immediately and refresh keeps valid session');
